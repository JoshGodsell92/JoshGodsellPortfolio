//////////////////////////////////////////////////////////////////////////
///File name: AI_WorldState.cs
///Date Created: 08/10/2020
///Created by: JG
///Brief: World State as known by the AI agent for making plan against.
///Last Edited by: JG
///Last Edited on: 30/10/2020
//////////////////////////////////////////////////////////////////////////
...
public class AI_WorldState : MonoBehaviour, WorldState
{
    ...
    private bool m_bHasPatrolled = false;
    private bool m_bKnowsPlayer = false;
    private bool m_bPlayerDefeated = true;
    private bool m_bCanGrab = true;
    private bool m_bActiveSearch = false;
    private bool m_bSearchNearbyHide = false;
    private bool m_bPlayerGrabbed = false;

    private bool m_bImprovedChances = true;

    private bool[] m_bObjectStates;
    ...
    public void Initialise()
    {
        ...
        m_WorldData.Add(new KeyValuePair<string, bool>("HasPatrolled", m_bHasPatrolled));
        m_WorldData.Add(new KeyValuePair<string, bool>("Investigating", m_bInvestigating));
        ...
        m_WorldData.Add(new KeyValuePair<string, bool>("CanGrab", m_bCanGrab));
        m_WorldData.Add(new KeyValuePair<string, bool>("ActiveSearch", m_bActiveSearch));
        m_WorldData.Add(new KeyValuePair<string, bool>("SearchNearbyHide", m_bSearchNearbyHide));
        m_WorldData.Add(new KeyValuePair<string, bool>("PlayerGrabbed", m_bPlayerGrabbed));

        m_WorldData.Add(new KeyValuePair<string, bool>("ImprovedChances", m_bImprovedChances));

        GetWorldObjectStates();
    ...
    //creates a goal state from a data set
    ...
    public void GetWorldObjectStates()
    {
        AI_ObjAction[] ActionObjects = FindObjectsOfType<AI_ObjAction>();

        m_bObjectStates = new bool[ActionObjects.Length];

        int t_iIndex = 0;

        foreach(AI_ObjAction ActObject in ActionObjects)
        {

            ActObject.SetID(t_iIndex);

            ActObject.AddIndexPreCondition(t_iIndex);

            m_WorldData.Add(new KeyValuePair<string, bool>("Object" + ActObject.GetID().ToString(), m_bObjectStates[t_iIndex]));
